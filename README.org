...
If a responsibility that has low abstraction is implemented in the code using high abstraction, the result is that code can appear unnecessarily fragmented, which will make the code hard to understand.
For example, if the "flip that bit that starts ignition" (low abstraction) is implemented with the help of varios classes in a namespace implementing a function (high abstraction),
the resulting code will look unnecessarily complicated!
...
having a nice (nice = responsibilites are implemented by a tigh set of source code) structuring of the code should help with being able to identify a sensible location for the implementation of additional responsiblities.
The implementation of the changed requirement might need to touch a few places; if this is the case, refactoring to keep responsibilites close might make sense.
Often times a set of responsibilities are too much intermingled: they are effectively making it impossible to have every responsiblity closely together.
The decision which responsibility to implement scattered (i.e., not closely together), while keeping the other responsibilities together is also a task for an architect.
...
The reason for why software can be viewed in terms of layers (as opposed to arbitrary directed graphs) is that one of the fundamental rules is to keep the "uses" dependencies acyclic.
...
Exactly how the two systems interact can be modelled, i.e., interface, can be designed in various ways.
...
- etc. :: there is an almost unlimited way of thinking of possible interfaces that can be used by code to communicate.
  See also "Appendix A Interfaces".

The code architects are deciding how to layer, and which interfaces are needed / used for the layers to communicate.
...
The general guideline is that side-effects shall happen as high as possible in the call stack.
...
While documenting the function can help, documentation can be wrong, incomplete or just plain missing, while the source code is the truth.
